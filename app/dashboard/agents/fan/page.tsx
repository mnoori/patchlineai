...
// Update PlatformEngagement type to be more flexible
...
type SelectedItem = PlatformEngagement | AudienceSegment | Campaign
...
export default function FanAgentPage() {
  ...
  // Type guards to check selectedItem type
  const isPlatformEngagement = (item: SelectedItem | null): item is PlatformEngagement => {
    return item !== null && 'platform' in item && 'metrics' in item
  }

  const isAudienceSegment = (item: SelectedItem | null): item is AudienceSegment => {
    return item !== null && 'size' in item && 'demographics' in item
  }

  const isCampaign = (item: SelectedItem | null): item is Campaign => {
    return item !== null && 'status' in item && 'performance' in item
  }

  // Reinitialize Instagram embeds when drawer opens
  useEffect(() => {
    if (openDrawer && isPlatformEngagement(selectedItem) && selectedItem.platform === "Instagram") {
      // Give the DOM time to render, then process embeds
      setTimeout(() => {
  ...
  const handleItemClick = (item: PlatformEngagement | AudienceSegment | Campaign, type: string) => {
    setSelectedItem(item)
    setDrawerType(type)
    setOpenDrawer(true)
  }

  const handleCreateContent = (platform: string, contentType: string) => {
  ...
  const handleTakeAction = (action: any, platform: string) => {
  ...
  const handleViewCalendarItem = (item: any) => {
    ...
    <div className="space-y-8">
                      ...
                      <div key={i} className="flex items-start gap-2 text-sm">
                        ...
                        <div className="flex-1 text-xs">
                          <div className="flex items-center gap-1">
                            ...
                            {'trending' in activity && activity.trending && <TrendingUp className="h-3 w-3 text-red-500" />}
                          </div>
      ...
      <Sheet open={openDrawer && drawerType === "platform"} onOpenChange={setOpenDrawer}>
        <SheetContent className="sm:max-w-md overflow-y-auto">
          <SheetHeader>
            <div className="flex items-center gap-2">
              {isPlatformEngagement(selectedItem) && selectedItem.icon}
              <SheetTitle>{isPlatformEngagement(selectedItem) ? selectedItem.platform : 'Details'}</SheetTitle>
            </div>
            <SheetDescription>Engagement insights and actions</SheetDescription>
          </SheetHeader>

          {selectedItem && drawerType === "platform" && isPlatformEngagement(selectedItem) && (
            <div className="mt-6 space-y-6">
              {/* Instagram Profile Embed - Only show for Instagram */}
              {selectedItem.platform === "Instagram" && (
                <div className="space-y-4">
                  <div className="relative rounded-lg overflow-hidden border border-border/50 bg-gradient-to-br from-pink-500/5 via-purple-500/5 to-orange-500/5">
                    {/* Instagram Embed Container - Full Width */}
                    <InstagramEmbed />
                  </div>
                  
                  {/* Quick Actions for Instagram Profile */}
                  <div className="flex gap-2">
                    <Button 
                      variant="outline" 
                      size="sm" 
                      className="flex-1 text-xs"
                      onClick={() => window.open('https://www.instagram.com/algoryxmusic/', '_blank')}
                    >
                    ...
                    <Button 
                      ...
                      className="flex-1 text-xs"
                      onClick={() => {
                        handleCreateContent('Instagram', 'story')
                      }}
                    >
      ...
      <Sheet open={openDrawer && drawerType === "segment"} onOpenChange={setOpenDrawer}>
        <SheetContent className="sm:max-w-md overflow-y-auto">
          ...
          {selectedItem && drawerType === "segment" && isAudienceSegment(selectedItem) && (
            <div className="mt-6 space-y-6">
              <div className="flex justify-between items-start">
                <div>
      ...
      <Sheet open={openDrawer && drawerType === "campaign"} onOpenChange={setOpenDrawer}>
        <SheetContent className="sm:max-w-md overflow-y-auto">
          ...
          {selectedItem && drawerType === "campaign" && isCampaign(selectedItem) && (
            <div className="mt-6 space-y-6">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-medium text-lg">{selectedItem.name}</h3>
