...
import { ChevronRight, ChevronDown, Loader2, Sparkles, RefreshCw, AlertCircle } from "lucide-react";
...
Your content should follow proper markdown formatting with headers, lists, and emphasis where appropriate.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Prepare the prompt with keywords as array and include the custom prompt text
      const finalPrompt: ContentPrompt = {
        ...prompt,
        keywords: keywords.split(",").map(k => k.trim()).filter(k => k),
  ...
  const StepIndicator = () => (
    <div className="hidden lg:block h-auto fixed right-10 top-1/2 -translate-y-1/2 z-10">
      <div className="flex flex-col items-center gap-4 p-3 rounded-lg relative">
        ...
        <div className={cn(
          "absolute w-[2px] h-full top-0 left-1/2 transform -translate-x-1/2 bg-muted",
        )}></div>
        ...
        <div className={cn(
          "absolute w-1 top-[33%] left-1/2 transform -translate-x-1/2 h-1 rounded-full bg-muted z-[5]",
        )}></div>
        
        <div className={cn(
          "absolute w-1 top-[66%] left-1/2 transform -translate-x-1/2 h-1 rounded-full bg-muted z-[5]",
        )}></div>
    ...
    <div ref={formRef} className="relative w-full pt-6">
      ...
      <Card className="w-full max-w-5xl mx-auto mb-8">
        <CardHeader>
          ...
          <CardDescription>
            Create AI-generated content with your specifications
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            >
              ...
              {currentStep === 1 ? (
                ...
                <div className="flex-1 rounded-lg border border-cosmic-teal/20 bg-cosmic-midnight/30 p-3 text-sm">
                  <p>After clicking "Generate Prompt", you'll see the prompt that will be sent to the AI. You can review and edit it before generating content.</p>
                </div>
              )}
              ...
              {currentStep === 2 && (
                <div className="flex space-x-3 pt-4">
                  ...
                  <Button 
                    ...
                    className="min-w-[140px] bg-cosmic-teal hover:bg-cosmic-teal/90 text-black"
                  >
                    {isGeneratingPrompt ? (
                      <>
                ...
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-2 col-span-1">
                    <div className="flex justify-between items-center">
                      <Label htmlFor="ai-prompt" className="text-base font-medium">AI Prompt</Label>
                      {hasGeneratedOnce && (
                        <Button 
                          variant="outline" 
                          size="sm" 
                          ...
                          className="h-8 gap-1"
                        >
                          {isGeneratingPrompt ? <Loader2 className="h-3 w-3 animate-spin" /> : <RefreshCw className="h-3 w-3" />}
                          Regenerate
                        </Button>
                      )}
                    </div>
                  ...
                  <div className="flex-1 rounded-lg border border-cosmic-teal/20 bg-cosmic-midnight/30 p-3 text-sm">
                    <p>This is the prompt that will be sent to the AI model. You can edit it to customize the content generation.</p>
                  </div>
                </div>
              </div>
