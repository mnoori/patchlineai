...
interface ArtistDetailDrawerProps {
  ...
  onAddToRoster?: (artist: Artist) => void
  roster?: any[]
}
...
export function ArtistDetailDrawer({
  ...
  const isInRoster = roster.some(rosterArtist => 
  ...
  const handleAddToRoster = () => {
    if (onAddToRoster && !isInRoster) {
      onAddToRoster(artist)
      setAddedToRoster(true)
      // Reset after animation
      setTimeout(() => setAddedToRoster(false), 3000)
    }
  }
    ...
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto bg-background/95 backdrop-blur-xl border-l border-border/50">
        <SheetHeader className="space-y-4">
          ...
          {/* Add to Roster Button - Prominent Placement */}
          {onAddToRoster && (
            <motion.div
          ...
          <div className="flex flex-wrap gap-2">
            {(Array.isArray(artist.platforms) ? artist.platforms : Object.keys(artist.platforms ?? {})).map((platform) => (
              <Badge key={platform} variant="outline" className="flex items-center gap-1 px-3 py-1 backdrop-blur-sm bg-white/5">
