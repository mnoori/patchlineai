 ...
 * DynamoDB Utilities
 * 
 * Utility functions for working with DynamoDB safely, including
 * handling of reserved keywords and common database operations.
 */

/**
 * DynamoDB Reserved Keywords that need special handling
...
export const RESERVED_KEYWORDS = [
  ...
  'WRITE', 'YEAR', 'ZONE',
  // Add any additional keywords causing issues in lowercase
  'status', 'url', 'slug'
].map(keyword => keyword.toLowerCase());

/**
 * Checks if an attribute name is a reserved keyword in DynamoDB
 * @param attributeName The attribute name to check
 * @returns true if the attribute is a reserved keyword
 */
export function isReservedKeyword(attributeName: string): boolean {
  return RESERVED_KEYWORDS.includes(attributeName.toLowerCase());
}

/**
 * Creates ExpressionAttributeNames map for a QueryCommand or other operations
 * that need to reference potential reserved keywords
 * @param attributes Array of attribute names that might be reserved
 * @returns An ExpressionAttributeNames object mapping each reserved attribute to a safe name
 */
export function createExpressionAttributeNames(attributes: string[]): Record<string, string> {
  const result: Record<string, string> = {};
  
  attributes.forEach(attr => {
    if (isReservedKeyword(attr)) {
      result[`#${attr}`] = attr;
    }
  });
...
/**
 * Safely builds a filter expression string with attribute names that might be reserved
 * @param attrName The attribute name to use in the expression
 * @param operator The comparison operator (=, <, >, etc)
 * @param placeholder The placeholder for the value (e.g. :value)
 * @returns A safe filter expression string
 */
export function buildSafeFilterExpression(
  attrName: string, 
  operator: string, 
  placeholder: string
): string {
  const safeAttrName = isReservedKeyword(attrName) ? `#${attrName}` : attrName;
  return `${safeAttrName} ${operator} ${placeholder}`;
} 