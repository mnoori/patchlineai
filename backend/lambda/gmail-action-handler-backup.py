...
def get_user_gmail_service(user_id: str):
    ...
    try:
        ...
        credentials = Credentials(
            ...
            scopes=item.get('scopes', ['https://www.googleapis.com/auth/gmail.readonly', 'https://www.googleapis.com/auth/gmail.send'])
        )
...
def handle_search_emails(user_id: str, request_body: Dict) -> Dict:
    ...
    try:
        ...
        # Parse request
        query = request_body.get('content', {}).get('application/json', {}).get('query', '')
        max_results = request_body.get('content', {}).get('application/json', {}).get('maxResults', 10)
        ...
        for msg in messages:
            try:
                message = service.users().messages().get(
                    userId='me',
                    id=msg['id'],
                    format='metadata'  # Fetch only metadata for search results (Subject, From, Date, Snippet)
                ).execute()
                # Extract email data
                ...
                email_data.append({
                    ...
                    # Removed 'body' to match OpenAPI schema for search results
                })
                # Store in Knowledge Base (fetch full message separately for KB)
                full_message_for_kb = service.users().messages().get(userId='me', id=msg['id'], format='full').execute()
                kb_body = extract_email_body(full_message_for_kb['payload'])
                store_email_in_knowledge_base(user_id, msg['id'], {
                    'subject': subject,
                    'from': from_email,
                    'date': date,
                    'body': kb_body
        ...
        return create_response(200, {
            ...
            # Removed 'query' to match OpenAPI schema
        }, '/search-emails', 'POST')
    except Exception as e:
...
def handle_read_email(user_id: str, request_body: Dict) -> Dict:
    ...
    try:
        service = get_user_gmail_service(user_id)
        email_id = request_body.get('content', {}).get('application/json', {}).get('emailId', '')
        if not email_id:
